...
* Girls named Max 
* Names that were used by exactly 5 or 6 children in 1990 ###########!!!&(/"§&()/!"§%z3uedfg)
...
#### das geht nich
filter(babynames, year==1990, n %in% c(5, 6)) %>% 
  head()
filter(babynames, year==1990 & n==(5) | year==1990 & n==6) %>% 
  head()
filter(babynames, year==1990, n==(5|6)) %>% 
  head()
...
###### hmmm da wissma ned weidaa
babynames %>% 
  group_by(name, sex) %>% 
  mutate(rank_ns = min_rank(desc(n))) %>% 
  mutate(med_rank_ns = median(rank_ns))
  arrange(med_rank_ns) %>% 
  head()
...
babynames %>% 
  mutate(med_rank = min_rank(desc(n))) %>% 
  arrange(med_rank) %>% 
  head()
