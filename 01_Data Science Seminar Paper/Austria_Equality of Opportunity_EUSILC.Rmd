...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
AT_fitControl <- trainControl(method = "cv", number = 10)
AT_cctree <- train(formula, data = AT_train, method = "ctree", trControl = AT_fitControl, na.action = na.pass)

AT_cctree #This is the suggested tree we get from applying Caret
...
FR_cct <- ctree(formula, data = FR_train, mincriterion = 0.5) #Using the suggestion we generate a Conditional Inference Tree and plot it as our final result
...
ES_cctree <- train(formula, data = ES_train, method = "ctree", trControl = ES_fitControl, na.action = na.pass)
