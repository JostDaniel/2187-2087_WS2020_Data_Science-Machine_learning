...
AT_equality_data <- AT_equality_data %>%
  mutate(train_index = sample(c("train", "test"), nrow(AT_equality_data), replace=TRUE, prob=c(0.8, 0.2)))
...
plot(AT_test$P_AtCt, AT_test$log_income)
```
...
ggplot(importance_cf, aes(x = var_name, y = importance)) +
    ...
    labs(title = "Conditional Random Forest variable importance", subtitle = "presented as the mean decrease in the sum of squared residuals when the variable is included in a tree split", x = "", y = "Mean decrease in sum of squared residuals") +
    coord_flip() +
