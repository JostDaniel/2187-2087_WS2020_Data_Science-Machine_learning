...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
test$prediction_tree <- predict(tree, newdata = test, type = c())

test$prediction_tree <- as.factor(test$prediction_tree)
test$income_K <- as.factor(test$income_K)

class(test$income_K)
 class(test$prediction_tree)

confusion <- confusionMatrix(test$income_K, test$prediction_tree, mode="sens_spec")
confusion


summary(test$prediction_tree)
