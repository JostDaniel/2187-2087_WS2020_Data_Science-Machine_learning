...
#netherlands_survey_clean$income_K <- as.factor(netherlands_survey_clean$income_K)                         
```
# **Research Question**
...
test$prediction_Rtree <- predict(Rtree, newdata = test, type = c())

test$prediction_Rtree <- as.factor(test$prediction_Rtree)
test$income_K <- as.factor(test$income_K)

class(test$income_K)
class(test$prediction_Rtree)

confusionR <- confusionMatrix(test$income_K, test$prediction_Rtree, mode="sens_spec")
confusionR


summary(test$prediction_Rtree)
summary(test$income_K)


test$prediction_Rtree <- predict(Rtree, newdata = test, type = c("class"))
confusionR <- confusionMatrix(test$vote, test$prediction_Rtree,positive = "ÖVP", mode="sens_spec")
confusionR
