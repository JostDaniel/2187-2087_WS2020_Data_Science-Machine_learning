...
Building upon this data set we further limit our data by age. We only include respondents aged between 30 and 59 since this captures the working age population. In the next step we drop all answers where the respondents refused or were not able to provide information concerning the intergenerational circumstances like e.g. father or mother presence. We do not do this for all variables since it would leave us with too little observations (e.g. dropping adults). Furthermore, the Statistik Austria data set contains more factorial entries for certain variables, then were applied by -@brunori18 , therefore we change the levels of certain variables.
...
ggplotly(varimpo2)
```
Similarly to our previous results, a persons origin or country of birth of either parent is an important circumstance in predicting the income of that perons. In difference to our previous results, the respondents sex appears to be of great importante. We think, that this could be due to the fact, that in the synthetic data, sex is the only variable next to income with the least missing entries. Other than that, the variable importance estimate is similar to the ones obtained using the data from 2019.
