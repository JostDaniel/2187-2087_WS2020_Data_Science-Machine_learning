...
data19 <- data19 %>%
  mutate(train_index = sample(c("train", "test"), nrow(data19), replace=TRUE, prob=c(0.75, 0.25)))

train <- data19 %>% filter(train_index=="train")
test <- data19 %>% filter(train_index=="test")

tree <- rpart(formula, data = train, cp=.008)
tree
...
caret_ctree <- train(formula, data = train, method = "ctree", trControl = fitControl, na.action = na.exclude)
caret_rpart <- train(formula, data = train, method = "rpart", trControl = fitControl, na.action = na.exclude)
caret_ctree
caret_rpart
```
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
test$AT_testpred <- predict(caret_ctree, test, type = "raw")

test$AT_testpred <- as.factor(test$AT_testpred)
test$inc_net_log <- as.factor(test$inc_net_log)

CM <- confusionMatrix(test$AT_testpred, test$inc_net_log)
