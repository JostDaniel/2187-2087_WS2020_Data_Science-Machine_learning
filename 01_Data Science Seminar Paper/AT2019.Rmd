...
data19 <- data19 %>% mutate(income = Winsorize(data19$net_income, minval = 1, maxval = quantile[2], probs = c(0.05, 0.95),
...
AT_equality_data <- AT_equality_data %>% select(
  PB140, HY020, PB150, PB220A, PT010, PT020, PT030, PT060, PT070, PT090, PT100, PT110, PT120, PT130, PT160, PT150, PT180, PT140, PT170, PT210, PY200G) %>% mutate(
    age = (2011 - PB140), log_income = log(HY020 + 1) #We add 1 so the logarithm works
  ) %>% filter(
    age %in% (27:59)
  ) %>% mutate(
...
FR_equality_data <- FR_equality_data %>% select(
  PB140, HY020, PB150, PB220A, PT010, PT020, PT030, PT060, PT070, PT090, PT100, PT110, PT120, PT130, PT160, PT150, PT180, PT140, PT170, PT210, PY200G) %>% mutate(
    age = (2011 - PB140), log_income = log(HY020 + 1)
  ) %>% filter(
    age %in% (27:59)
  ) %>% mutate(
...
# # Unfortunately the synthetic household data set for Italy does not contain enough entries. 
# # Therefore, we use gross monthly income, as in the Austria 2019 data set, and multiply it by 12
# 
# IT_equality_data <- IT_personal_data %>% select(
#   PB140, PY200G, PB150, PB220A, PT010, PT020, PT030, PT060, PT070, PT090, PT100, PT110, PT120, PT130, PT160, PT150, PT180, PT140, PT170, PT210, PY200G) %>% mutate(
#     age = (2011 - PB140), log_income = log(PY200G * 12), annual_income = (PY200G * 12)
#   ) %>% filter(
#     age %in% (27:59)
#   ) %>% mutate(
#     citizenship = factor(PB220A, labels = c(1,2,3))
