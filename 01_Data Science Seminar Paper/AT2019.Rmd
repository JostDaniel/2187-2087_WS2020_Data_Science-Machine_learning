...
**TO DOs**
* AT2019 wrangling und recoded fertig
* AT2011 Wrangling Code vereinfachen (ist ja für alle Länder gleich)
...
DANIEL:
  ...
  2.  DAN: Income schau ich mir auch noch an, ob wir da nit annual income verwenden können
  3.  Plots (siehe unten)
  5.  Daniel, kannst du die Age pyramide fixen, oder noch einfacher, sie einfach nach vorne verschieben? :D
**Ideen für Plots**
...
data19 <- data19 %>% 
  ...
  filter(age %in% (30:59), mother_present >= 0, father_present >= 0) %>% 
...
levels(data19$father_edu) <- c("Army", "Manager", "Professional", "Technician", "Clerical", "Service","Agri", "Craft", "Operator", "Elementary")
levels(data19$mother_edu) <- c("Army", "Manager", "Professional", "Technician", "Clerical", "Service","Agri", "Craft", "Operator", "Elementary")
```
...
<!-- Man kann von kategoriale Variablen (auch wenn ordinalskaliert) nicht einfach den mean berechnen, also können schon ist aber eher nix. jaaa macht sinn..! --> 
...
agepyra <- ggplot(data19, aes(x = age, fill=sex))  + 
  ...
  scale_x_continuous(breaks = seq(30,59,2), labels=abs(seq(30,59,2))) +
...
data19cor <- sapply(data19cor, as.numeric)
...
Empirical approach as described in [@brunori18, 4]:
...
exp(9.630)
```
We obtain a deep tree with 10 inner nodes and 11 terminal nodes. In this estimate the citizenship of the father appears to be the first determining split. The predictive power of the model does not appear to be good, when we look at the errors indicated in the grey boxes. We obtain a MSE for the model of 0,52. As an example for reading the tree, we point at terminal node `8`, where we see that 422 observations are grouped together. Going down the tree from the initial split, we can tell that this group has a father with Austrian citizenship, employed, a mother with Austrian citizenship, and one parent present at home. The predicted outcome for income for this group, which accounts for 8% of the whole population sample, is EUR 44311,52. Going, down the other split to node `6` we find the largest group accounting for 58% of the whole sample. Here, the respondents are grouped together, where both or none of the parents were present, and there were at most 2 working adults present in the household. The predicted outcome for this group is EUR 49961,1.
...
caret_ctree_B <- ctree(formula, data = data19, control = ctree_control(testtype = "Univariate", alpha = 0.05, mincriterion = 0.99)) 
caret_ctree_B
...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
cf <- cforest(formula, train, na.action = na.pass, control = ctree_control(teststat = "quadratic", testtype = "Univariate", mincriterion = 0.99), ntree = 500L, perturb = list(replace = T, fraction = 0.8))
...
ggplotly(varimpo)
```
Both the _conditional inference tree_ and the _conditional inference forest_ analysis indicate that sex, is the most important determinant of an individuals income in Austria. We are surprised by this outcome, since [@brunori18] find that for Austria, the country of birth of the father is the variable determining the initial split of the tree. However, the variable is significantly more important than any other variable under consideration, and while it is arguably not a generationally transmittable characteristic, it is genetic, circumstantial, and completely out of the respondents control to influence. 
...
Outcome Variables i.e. Income: Total Household gross income (HY020), Total Disposable Income (HY020), Dwelling Type (HH010), Housing (HH030). 

We first use more variables than ultimately used in the analysis. We use the year of birth to calculate the age, and then exclude everyone older than 60 or younger than 27, as was done in the paper we are referring to in order to focus on the working population only. First, we included both monthly and annual gross income. But in this cross-country analysis we use annual gross income as our outcome variable. 
...
```{r}
quantile_AT <- quantile(AT_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

AT_equality_data <- AT_equality_data %>% mutate(income = Winsorize(AT_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))
```
...
quantile_FR <- quantile(FR_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

FR_equality_data <- FR_equality_data %>% mutate(income = Winsorize(FR_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))
...
quantile_DK <- quantile(DK_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

DK_equality_data <- DK_equality_data %>% mutate(income = Winsorize(DK_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))
...
quantile_ES <- quantile(ES_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

ES_equality_data <- ES_equality_data %>% mutate(income = Winsorize(ES_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))
...
quantile_FI <- quantile(FI_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

FI_equality_data <- FI_equality_data %>% mutate(income = Winsorize(FI_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))
...
quantile_LV <- quantile(LV_equality_data$annual_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

LV_equality_data <- LV_equality_data %>% mutate(income = Winsorize(LV_equality_data$annual_income, minval = 1, maxval = quantile_AT[11], probs = c(0.05, 0.95), na.rm = FALSE, type = 7), inc_log = log(income))

```

```{r}
levels(FR_equality_data$sex)               <- c("Male", "Female")
levels(FR_equality_data$father_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(FR_equality_data$mother_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(FR_equality_data$father_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(FR_equality_data$mother_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(FR_equality_data$father_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(FR_equality_data$mother_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(FR_equality_data$father_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(FR_equality_data$mother_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(FR_equality_data$father_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(FR_equality_data$mother_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(FR_equality_data$tenancy)           <- c("Don't know", "Owner", "Tenant", "Free Acc")
levels(FR_equality_data$father_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
levels(FR_equality_data$mother_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")

levels(DK_equality_data$sex)               <- c("Male", "Female")
levels(DK_equality_data$father_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(DK_equality_data$mother_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(DK_equality_data$father_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(DK_equality_data$mother_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(DK_equality_data$father_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(DK_equality_data$mother_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(DK_equality_data$father_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(DK_equality_data$mother_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(DK_equality_data$father_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(DK_equality_data$mother_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(DK_equality_data$tenancy)           <- c("Don't know", "Owner", "Tenant", "Free Acc")
levels(DK_equality_data$father_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
levels(DK_equality_data$mother_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")

levels(FI_equality_data$sex)               <- c("Male", "Female")
levels(FI_equality_data$father_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(FI_equality_data$mother_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(FI_equality_data$father_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(FI_equality_data$mother_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(FI_equality_data$father_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(FI_equality_data$mother_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(FI_equality_data$father_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(FI_equality_data$mother_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(FI_equality_data$father_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(FI_equality_data$mother_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(FI_equality_data$tenancy)           <- c("Don't know", "Owner", "Tenant", "Free Acc")
levels(FI_equality_data$father_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
levels(FI_equality_data$mother_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")

levels(ES_equality_data$sex)               <- c("Male", "Female")
levels(ES_equality_data$father_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(ES_equality_data$mother_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(ES_equality_data$father_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(ES_equality_data$mother_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country", "Non-EU")
levels(ES_equality_data$father_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(ES_equality_data$mother_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(ES_equality_data$father_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(ES_equality_data$mother_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(ES_equality_data$father_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(ES_equality_data$mother_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(ES_equality_data$tenancy)           <- c("Don't know", "Owner", "Tenant", "Free Acc")
levels(ES_equality_data$father_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
levels(ES_equality_data$mother_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")

levels(LV_equality_data$sex)               <- c("Male", "Female")
levels(LV_equality_data$father_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(LV_equality_data$mother_cit)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(LV_equality_data$father_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(LV_equality_data$mother_cob)        <- c("Don't know","Resp. Present Country","Other EU country","Other European Country")
levels(LV_equality_data$father_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(LV_equality_data$mother_edu)        <- c("Don't know", "None", "Low", "Medium", "High")
levels(LV_equality_data$father_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(LV_equality_data$mother_manag)      <- c("Don't know", "Supervisory", "Non-supervisory")
levels(LV_equality_data$father_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(LV_equality_data$mother_occup_stat) <- c("Don't know", "Employed", "Self-employed", "Unemployed", "Retired etc.", "Domestic", "Other inactive")
levels(LV_equality_data$tenancy)           <- c("Don't know", "Owner", "Tenant", "Free Acc")
levels(LV_equality_data$father_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
levels(LV_equality_data$mother_occup)      <- c("Don't know", "Armed forces", "Manager", "Professional", "Technician", "Clerical", "Service",                                                  "Agri", "Craft", "Operator", "Elementary")
```
...
Summary_Stat_all %>% 
  ...
  scale_fill_manual(values = c("gray40", "gray40", "gray40", "red", "gray40", "gray40")) +
...
### Regression Trees
Following [@brunori18] and the standard procedure for CART we split the data into training and testing data by $2/3:1/3$.
...
formula <- inc_log ~ sex + parents_present + adults_home + children_home + father_cob + father_cit + mother_cob + mother_cit + father_edu + mother_edu + father_occup_stat + mother_occup_stat + father_occup + mother_occup + father_manag + mother_manag + tenancy
```
...
AT_ctree2 <- ctree(formula, data = AT_train, control = ctree_control(testtype = "Univariate", mincriterion = 0.99, maxdepth = 5)) 
AT_ctree2
...
AT_test$perror <- (AT_test$P_AtCt - AT_test$inc_log)^2

AT_test$RMSE <- sqrt(sum((AT_test$P_AtCt - AT_test$inc_log)^2/nrow(AT_test), na.rm = T))
...
FR_test$perror <- (FR_test$P_FRCt - FR_test$inc_log)^2

FR_test$RMSE <- sqrt(sum((FR_test$P_FRCt - FR_test$inc_log)^2/nrow(FR_test), na.rm = T))
...
### Conditional Inference Forest
```{r}
AT_cf <- cforest(formula, AT_equality_data, na.action = na.pass, control = ctree_control(teststat = "quadratic", testtype = "Univariate", mincriterion = 0.99), ytrafo = NULL, scores = NULL, ntree = 500L, perturb = list(replace = FALSE, fraction = 0.8))
