...
Confusion Matrix (Problem since regression and not classification - need some other measure of model predictive performance), ROC etc.
Graphs
...
summary(data19) %>% head()
summary(data19$inc_net)

plot(data19$inc_net)

data19$inc_classes <- with(data19, ifelse(inc_net >= 10000, 1,ifelse(inc_net >= 9000,2, ifelse(inc_net >= 8000,3, ifelse(inc_net >= 7000,4, ifelse(inc_net >= 6000,5, ifelse(inc_net >= 5000,6, ifelse(inc_net >= 4000,7, ifelse(inc_net >= 3000,8, ifelse(inc_net >= 2000,9,ifelse(inc_net >= 1000,10, 11)))))))))))

levels(data19$inc_classes) <- c("first_class", "second_class", "third_class", "fourth_class", "fifth_class", "sixth_class", "seventh_class", "eigth_class", "ninth_class", "tenth_class", "eleventh_class" )

data19$inc_classes 



data19$inc_classes <- as.factor(data19$inc_classes)

levels(data19$inc_classes)
...
formula = inc_classes ~ country_birth + citizenship + father_cit + father_edu + father_occup_stat + father_occup + father_manag + mother_cit + mother_edu + mother_occup_stat + mother_occup + mother_manag + tenancy + children + adults + adults_working + both_parents_present
...
test$prediction_tree <- predict(tree_1, newdata = test, type = c("class"))

class(test$prediction_tree)
levels(test$prediction_tree)

confusion <- confusionMatrix(test$inc_classes, test$prediction_tree, mode="sens_spec")
confusion
...
```{r}
fitControl <- trainControl(method = "repeatedcv", number = 10, repeats = 10, savePredictions = T, classProbs = T, summaryFunction = multiClassSummary)
...
caret_rpart <- train(formula, data = data19, method = "rpart", trControl = fitControl, tuneGrid = tuning_grid, metric = "Accuracy", na.action = na.pass)
...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
test$prediction_tree_caret <- predict.train(caret_rpart, newdata = test, type = c("raw"), na.action = na.rpart)

test$prediction_ctree <- predict.train(caret_ctree, newdata = test, type = c("raw"), na.action = na.rpart)

class(test$prediction_tree_caret)
class(test$prediction_tree_ctree)

#round(test$prediction_tree_caret,0)

test$prediction_tree_caret <- as.factor(test$prediction_tree_caret)
test$prediction_tree_ctree <- as.factor(test$prediction_tree_ctree)

CM_caret <- confusionMatrix(test$prediction_tree_caret, test$inc_classes,mode="sens_spec")
CM_caret

#geht nicht fehler schon bei prediction
CM_ctree <- confusionMatrix(test$prediction_tree_ctree,test$inc_classes, mode="sens_spec")
...
logit <- glm(formula, data = data19)

test$prediction_logit <- predict(lm, newdata = test, type = c("response"))

test <- test %>% 
  drop_na(prediction_lm) %>% 
  mutate(prediction_lm = as.factor(prediction_lm))


confusion <- confusionMatrix(test$prediction_lm,test$inc_net_log)
confusion

test$prediction_tree_caret_scores <- predict(caret_rpart, test ,type = c("raw"))
test$prediction_lm_scores <- predict(lm, test ,type = c("response"))

precrec_obj <- evalmod(scores = cbind(test$prediction_tree_scores,test$prediction_lm_scores), 
                       labels = cbind(test$survived,test$survived), 
                       modnames = c("classification tree","lm"),
...
*Random Forest*
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
AT_cforest <- cforest(formula, data19, na.action = na.pass, control = ctree_control())

vi <- varimp(AT_cforest)



AT_cforest
# cf_1 = AT_cforest$finalModel
# 
# pred_ATcf <- predict(AT_cforest, data19)

# AT_cftpred <- predict(AT_cforest, newdata = NULL)
# gettree(AT_cforest, tree = 2L)
# 
# plot(formula, data = data19)
# lines(AT_cfpred )
