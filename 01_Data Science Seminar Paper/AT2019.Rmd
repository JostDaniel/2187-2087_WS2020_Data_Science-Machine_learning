...
# fehlt: M001100 (Presence of parents at home) father, M001200 (mutter), M002000 - lebensform ohne eltern, M001500 mit anderen erwachsenen
...
data19 <- data19_p %>% 
  ...
  mutate("inc_net_log" = log(inc_net+3))    # logged net income per month of respondent
...
```{r}
# data19 %>% 
# filter(age>=25, age<=58) %>% 
# summarize(n = n(), min(age), max(age))
