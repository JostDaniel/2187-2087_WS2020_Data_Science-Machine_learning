...
data19 <- data19 %>% select(sex, hy020, P038004, P110000nu, P111010nu, alter, M009010, M010000, M014000, M016000, M017000, M020010, M021000, M025000, M027000, M028000, M004000, M001300, M001510, M003100, M001100, M001200, M002000, M001500) %>% 
  ...
  filter(age %in% (30:59), mother_present >= 0, father_present >= 0, father_cit >= 0, mother_cit >= 0) %>% 
  # DAN: Hier schau ich noch dass die observations nicht komplett gedropt werden wenn 
  mutate("both_parents_present" = father_present + mother_present,
        ...
        # Austria = 1 and Other = 2
        #father_manag = ifelse(father_manag == 1, 1, ifelse(2, 2, NA)),
        

        
        #mother_manag = ifelse(mother_manag == 2, 2, ifelse(1, 1, NA))
  )

# factorize cataegorial variable
factor <- c("sex",
            "country_birth", 
            "citizenship",           
            "father_cit", 
...
# recode variables
levels(data19$country_birth) <- c("Unknown", "Austria", "EU15", "EU12", "Jugo", "Turkey", "Other")
levels(data19$citizenship) <- c("Austria", "EU15", "EU12", "Jugo", "Turkey", "Other")
levels(data19$father_cit) <- c("Austria", "Other")
levels(data19$mother_cit) <- c("Austria", "Other")
# levels(data19$father_manag) <- c("Unknown", "Yes", "No")
```
...
```{r}
quantile(data19$net_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

data19 <- data19 %>% mutate(income = Winsorize(data19$net_income, minval = 1, maxval = 107645.6, probs = c(0.05, 0.95),
