...
formula = inc_net_log ~ country_birth + citizenship + father_cit + father_edu + father_occup_stat + father_occup + father_manag + mother_cit + mother_edu + mother_occup_stat + mother_occup + mother_manag + tenancy + children + adults + adults_working + both_parents_present
...
tree_1 <- rpart(formula, data = train, cp=.002)
tree_1
...
test$inc_net_log <- as.factor(test$inc_net_log)

# define classes before!
CM_caret <- confusionMatrix(test$prediction_tree_caret, test$inc_net_log)

CM_ctree <- confusionMatrix(test$prediction_tree_ctree,test$inc_net_log, mode="sens_spec")
