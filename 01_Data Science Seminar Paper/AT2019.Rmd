...
Inequality statistics tend to be heaviliy influenced by outliers, thus we first apply the winsorization function to our data [@brunori20, p12]. The winsorization sets all non-positive incomes equal to 1 and scales back all incomes exceeding the 95th percentile of the income distribution to that same threshold. Furthermore, we transform the income variable by taking the log. This means that ultimately our predicted output income, is the exponential of the predicted log income.
```{r}
quantile <- quantile(data19$net_income, weights = NULL, probs = seq(0, 1, 0.005), na.rm = FALSE, names = TRUE, type = 7) %>% tail(12)

data19 <- data19 %>% mutate(income = Winsorize(data19$net_income, minval = 1, maxval = quantile[11], probs = c(0.05, 0.95),
...
```{r}
median(data19$age)
median(data19$net_income)
mean(data19$net_income)
```
...
exp(9.630)
```
We obtain a deep tree with 10 inner nodes and 11 terminal nodes. In this estimate the citizenship of the father appears to be the first determining split. The predictive power of the model does not appear to be good, when we look at the errors indicated in the grey boxes. As an example for reading the tree, we point at terminal node `8`, where we see that 422 observations are grouped together. Going down the tree from the initial split, we can tell that this group has a father with Austrian citizenship, a mother with Austrian citizenship, and one parent present at home. The predicted outcome for income for this group is EUR 44311.52 (the second node, is tricky to interpret as non-contradicting descriptions of occupation status, are grouped into different outcomes). The predicted income for a person, whos father does not have Austrian citizenship, as in nodes 20 and 21, would be  EUR 15214.44 EUR 36643.82. 

In the next step, we cross-validate use cross validation for our prediction using the `caret` package, thus we also use the split of data (`test`, `training`).
...
# test$perror <- (test$P_AtCt - test$inc_log)^2
# test$RMSE <- sqrt(sum((test$P_AtCt - test$inc_log)^2/nrow(test), na.rm = T))
# head(test$RMSE)
# 
# 
# test$P_AtCt_caret <- predict(caret_rpart, newdata = as.data.frame(test))
# test$perror_caret <- (test$P_AtCt_caret - test$inc_log)^2
# test$RMSE_caret <- sqrt(sum((test$P_AtCt_caret - test$inc_log)^2/nrow(test), na.rm = T))
...
```{r echo=T, message=FALSE, error=FALSE, warning=FALSE}
cf <- cforest(formula, data19, na.action = na.pass, control = ctree_control(teststat = "quadratic", testtype = "Univariate", mincriterion = 0.99), ntree = 200L, perturb = list(replace = T, fraction = 0.8))
