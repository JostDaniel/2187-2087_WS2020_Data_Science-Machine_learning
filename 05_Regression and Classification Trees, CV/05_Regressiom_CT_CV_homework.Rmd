...
```{r}
train$prediction_tree <- predict(tree, newdata = train) 
...
# We can use this for the normal TREE, but not for the TREE_caret tuned version, not clear why

train$prediction_tree <- predict(TREE, newdata = train, type = "class") 

# We have to change the level of the vote variable to be the same as the outcome of the prediction
# train$vote <- train$vote %>% as.factor()

# 1st Confusion matrix
confusion1 <- confusionMatrix(train$vote , train$prediction_tree)
confusion1

# 2nd Confusion matrix
test$prediction_tree <- predict(TREE, newdata = test, type = "class")

confusion <- confusionMatrix(test$vote, test$prediction_tree)
confusion

#Confusion Matrix for Tree2
train$prediction_tree2 <- predict(tree2, newdata = train, type = "class")
confusion2 <- confusionMatrix(train$vote , train$prediction_tree2)
confusion2

test$prediction_tree2 <- predict(tree2, newdata = test, type = "class")
confusion2_1 <- confusionMatrix(test$vote , test$prediction_tree2)
confusion2_1
...
train$prediction_tree_caret1 <- predict(tree_caret1, newdata = train, type = "class")

test$vote <- test$vote%>%
  as.factor()

confusion2 <- confusionMatrix(test$vote, test$prediction_tree)
confusion2
```
...
control.vp <- trainControl(method = "repeatedcv", number = 10, repeats = 10, savePredictions = T, summaryFunction = twoClassSummary)
```
