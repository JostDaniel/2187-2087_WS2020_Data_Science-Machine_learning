...
## Seperate the voting data set into test and training data and changing the names
voting <- voting %>%
...
formula1 <- vote_outcome ~  gender + age + top_issue1 + financial_sit + polit_trust + gov_sat + close_to_eu + educ + income + left_or_right + immigration_neg + immigration_pos + religious_aff
...
tree_caret1 <- train(data = voting, formula1, method = "rpart", trControl = control, tuneGrid = tuning_grid, metric = "logLoss", na.action = na.pass)
tree_caret1

tree_caret1 <- tree_caret$finalModel
...
voting_test$prediction_tree <- predict.train(tree_caret, newdata = voting_test, type = "raw")
