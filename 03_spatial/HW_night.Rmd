...
```{r}
ggplot(sf_2) + geom_sf() + theme_void() + labs(title = "Regions and departments of France", subtitle = "Régions et départements français") + geom_sf(data = sf_1, alpha =.5)
```
...
plot_1 <- ggplot(sf_1_extract, aes(x = gdp_bl, y = nl_ml)) + 
  ...
  geom_text(data = ile, aes(label = "Paris <3"), color = "darkred", size = 3, vjust = 2) +
...
plot_2 <- ggplot(sf_2_extract, aes(x=gdp, y=nightlights)) + 
  geom_point() +
...
gdp_plot <- sf_1_extract %>% 
  ...
  scale_fill_viridis(option = "viridis", name = "GDP in thousands") +
  labs(title = "GDP") +
  theme_void() + 
...
-> \arrow Same projection, same extent and resolution for both layers :)
