...
data_path = "/Users/annevalder/Desktop/UNI /WU WIEN/WS 20:21/2187-2087_WS2020_Data_Science-Machine_learning/06_RandomForest_BoostedRegressiontree"
...
col_input %>% ggplot(aes(x=raster_pop_100)) +
  geom_density()

#raster_pop_100 = average pop density on 100 by 100 m level  (total density by area) to get pop density on grid level 
...
col_input %>% ggplot(aes(x=raster_pop_100, y=night_lights_100, color = night_lights_100)) + geom_point() + geom_smooth()
...
quindio_grid %>% ggplot(aes(x=night_lights_100, color = as.character(protected_areas_100))) + geom_density()
...
quindio_grid %>% ggplot(aes(x=night_lights_100,y= hydro_slo_100)) + geom_point()
...
```{r}
col_input <- col_input %>% mutate(raster_pop_100_log = log(raster_pop_100))
rf_1 <- ranger(data = col_input, dependent.variable.name = "raster_pop_100_log", importance = "impurity")
class(rf_1)"

I am not sure if this is helpful but have you tried putting the name of the depended variable in quotation marks --> f_1 <- ranger(data = col_input, dependent.variable.name = "raster_pop_100_log", importance = "impurity") maybe like that

col_input = col_input[,2:76]
col_input[,1] = log(col_input[,1])
rf_1 = ranger(data = col_input, dependent.variable.name = "raster_pop_100", importance = "impurity")
class(rf_1)
```
