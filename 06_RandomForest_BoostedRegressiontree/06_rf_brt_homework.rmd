...
quindio_grid %>% 
  ggplot(aes(x=night_lights_100,y= hydro_slo_100)) + 
...
# eval=FALSE -> do not run code in chunk damit geknitted werden kann. Sobald code funktioniert muss man das weggeben

# eig LOG schon vorher !
col_input <- col_input %>% 
  mutate(raster_pop_100_log = log(raster_pop_100))

head(col_input$raster_pop_100)
head(col_input$raster_pop_100_log)
# many negative numbers -> NA when logged    !
