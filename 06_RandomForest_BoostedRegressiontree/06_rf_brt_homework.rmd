...
# Next, we need a tuning grid, in which we specify which parameter we want to tune and the corresponding values which should be tried out. With caret and the ranger package we have 3 different parameters that we can tune: Mtry, the splitrule and the min-nod-size. Following the lecture we decide that Mtry is most important and should therefore be tuned. Mytr is the number of variables used by rf to split the tree, and it cannot be less than one or more than the number of columns of predictors. As criteria that should be used to find the optimal split points  we use variance. The min number of observations that are associated with each leave node gas to be at least 1 because at least one observations has to be in each leaf node.
