...
quindio_grid %>% 
  ggplot(aes(x=night_lights_100, color = as.character(protected_areas_100))) + 
  geom_density() +
  theme_classic()

quindio_grid %>% 
  ggplot(aes(x=night_lights_100, color = as.character(protected_areas_100), y = stat(count))) + 
  geom_point(alpha = 1/10, size = 0.5) +
  theme_classic()

quindio_grid %>% 
  ggplot(aes(x=night_lights_100, color = as.character(protected_areas_100), y = stat(count))) + 
  geom_dotplot(alpha = 1/10, size = 0.1) +
  theme_classic()
...
Use the input data for Colombia on the municipal level () to train a Random Forest. Use the log of the population density (raster_pop_100) as the dependent colombia_input.fstvariable. Tune one of the parameters in the Random Forest. Briefly describe the parameter you are tuning and explain why you have decided for this parameter.
