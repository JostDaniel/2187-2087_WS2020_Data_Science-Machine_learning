...
```{r}
# First,we want to use the the col_input data for all the estimations and tuning and then use quindio_grid for the predictions. This is because once we found a good tuned random forest and boosted regression tree, we then use the grid level data to estimate pop density on the grid level. We begin by removing the first column in col_input containing the geocode since we can not use it for the estimation because we cannot use it to explain population density

col_input <- col_input[,2:76]

# Taking the log of the dependent variable because as seen in class/ publications this works better for estimations on population density
...
# Next, we need a tuning grid, in which we specify which parameter we want to tune and the corresponding values which should be tried out. With caret and the ranger package we have 3 different parameters that we can tune: Mtry, the splitrule and the min-nod-size. Following the lecture we decide that Mtry is most important and should therefore be tuned. Mytr is the number of variables used by rf to split the tree, and it cannot be less than one or more than the number of columns of predictors. As criteria that should be used to find the optimal split points we use variance. The min number of observations that are associated with each leave node has to be at least 1 because at least one observations has to be in each leaf node.

# Considering the trade-off between how much parameters we want to try out and the time used to calculate the rf, we decided to alter the maximum value for the sequence in mytr and the the number of min.node.size by 1, compared to the model in class, to obtain a tuning grid with 35 instead of 9 observations of 3 variables. This gives us a better chance of finding an even better final model. 

tuning_grid = expand.grid(mtry = seq(20,50, by = 5), splitrule = "variance", min.node.size = seq(1, 5, by = 1))
head(tuning_grid)

# We now use caret and ranger to optimize the model and tune the parameters. We also include the model's importance scores (Check how important each variable is in explaning our dependent variable).

### HAT CA 15 Min zum rechnen bei mir gedauert!! aber dafür ists ein besseres model als in der VL!
...
# check RMSE and variable importance to compare to other models 

rf_1$prediction.error
head(importance(rf_1))
# From the output of the caret training we chose the model with the smallest RMSE since this is the best one. In this case it is the model with .... Then we save the final model from the caret package.  
...
# fehler noch ?
# only use columns with explanatory variables this is why we specify columns 5 to 78 

pred_rf_1 = predict(rf_1, data = quindio_grid [,5:78])
                      
quindio_grid

# Further to use the predictions for plots or analyses we first have to take the exponent, to reverse taking the log before the estimations. Moreover, we can combine them with information from the input data, like the coordinates of the corresponding grids, the geocodes, and the initial values of the average population density.
...
# To later compare the performance of all models, we can do a cross validation using the function xgb.cv that is included in the xgboost package.
# To optimize the model we combine the packages caret and xgboost.Again, we start this process by specifying which type of cross validation we are interested in. 

control = trainControl(method = "cv", number = 5)

# Then we create a tuning grid, setting the parameters that we want to tune and the corresponding values that should be tried out.

## abändern!!! & Erklärung warum welcher Parameter 
...
```{r}
# We can again use the columns with the explanatory variables in our data from Bogotá to create predictions.
...
```{r}
# First combine all predictions in one data frame.
...
ggplot() +
    geom_raster(pred_all, mapping = aes(x, y, fill = prediction)) +
    coord_fixed() +
    scale_fill_viridis(option = "plasma", trans = "log", breaks = c(3, 20, 150)) +
