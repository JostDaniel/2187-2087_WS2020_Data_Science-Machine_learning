...
# data_path = "/Users/Leonard/Desktop/2187-2087_WS2020_Data_Science-Machine_learning/06_RandomForest_BoostedRegressiontree"

# Read input data on municipal level for the whole country
col_input = read_fst("colombia_input.fst")
# col_input = read_fst(file.path(data_path, "data", "colombia_input.fst"))

# Read input data on grid level for the department Quindío
quindio_grid = read_fst("quindio_input_grid.fst")
...
```{r}
col_input %>% ggplot(aes(x=raster_pop_100)) +
  geom_density() + geom_point(aes(y = night_lights_100, alpha = night_lights_100))


ggplot(data = col_input) +
  geom_point(mapping = aes(x=raster_pop_100, y = night_lights_100, alpha = night_lights_100))+
  geom_smooth(mapping = aes(x=raster_pop_100, y = night_lights_100))
...
quindio_grid %>% ggplot(aes(night_lights_100, color = as.character(protected_areas_100))) + geom_bar()

quindio_grid %>% ggplot(data = night_lights_100) +
  geom_bar(mapping = aes(x = protected_areas_100))
```
